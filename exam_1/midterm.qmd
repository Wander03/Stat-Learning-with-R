...
knn_mod <- nearest_neighbor(neighbors = 26) %>%
  set_engine("kknn") %>%
  set_mode("classification")

knn_wflow <- workflow() %>%
  add_model(knn_mod) %>% 
  add_recipe(recipe_best_subset %>% step_normalize(all_numeric_predictors()))

knn_fit <- knn_wflow %>%
  fit(mvb_train)

predict_metrics(mvb_train, knn_fit)

predict_metrics(full_df %>% inner_join(mvb_test[,-1]), knn_fit)
```
...
identical(full_df %>% inner_join(mvb_test[,-1], by = colnames(mvb_test[,-1])) %>% select(-preference), mvb_test[,-1])
```
