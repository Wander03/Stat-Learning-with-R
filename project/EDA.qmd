...
rank <- rank %>%
  ...
  mutate(
    ...
    rank = ifelse(is.na(rank) | rank > 125, "above 125", rank),
    rank = factor(rank)
    ) %>%
  filter(year >= 2004) %>%
...
remove_cols <- prop_na %>%
  filter(Proportion_Missing >= 0.4, Proportion_Missing <= .5) %>%
  pull(Column)
...
joined_data_23_24 %>%
  select(university_name, all_of(remove_cols))
...
joined_data_23_24 %>%
  filter(is.na(CONTROL_PEPS)) %>%
  select(university_name,UNITID)

rank %>%
  filter(university_name == "Adelphi University")

card_23_24 %>%
  filter(INSTNM == "Adelphi University")

# look at naniar to see what columns have missing data 
...
if school has none, impute overall mean

do lasso
do something else for chosen variables
