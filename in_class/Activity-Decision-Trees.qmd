...
cann_clean <- cann_clean %>%
  bind_cols(predict(tree_fit, cann_clean, type = "prob")) %>%
  mutate(
    tree_pred = predict(tree_fit, cann_clean)$.pred_class
  )

roc_auc_tree <- cann_clean %>%
  roc_auc(
    truth = Type,
    c(.pred_hybrid, .pred_indica, .pred_sativa)
    )

precision_tree <- cann_clean %>%
  precision(
    truth = Type,
    estimate = tree_pred
  )

recall_tree <- cann_clean %>%
  recall(
    truth = Type,
    estimate = tree_pred
  )
...
The first level (root node) tells us that the majority of the data set is hybrid (51%), then if the strain is not sleepy this percentage increases, meanwhile if it is sleepy it is now more likely to be indica. 
